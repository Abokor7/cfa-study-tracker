...
const STATUS_OPTIONS = ['Planned', 'Done', 'Missed'];
const MOCK_OPTIONS = ['No', 'Yes'];
const WEEKLY_GOAL_DEFAULT = {
  hours: 28,
  questions: 250,
  accuracy: 75,
  mocks: 1,
...
const initialCompletedLogs = [
  {
    ...
    morningChecklist: {
      startMorningSession: true,
      mainTopicLearning: true,
      firstQuestionBlock: true,
      formulasReview: false,
    },
    afternoonChecklist: {
      startAfternoonSession: true,
      ethicsOrSecondTopic: true,
      secondQuestionBlock: true,
      recapAndErrorLog: false,
    },
  },
  {
    date: '2026-03-28',
...
function defaultMorningChecklist() {
  return {
    startMorningSession: false,
    mainTopicLearning: false,
    firstQuestionBlock: false,
    formulasReview: false,
  };
}

function defaultAfternoonChecklist() {
  return {
    startAfternoonSession: false,
    ethicsOrSecondTopic: false,
    secondQuestionBlock: false,
    recapAndErrorLog: false,
  };
}
...
function buildCalendarLogs() {
  return buildDateRange('2026-03-27', '2026-08-24').map((date) => {
    ...
    return {
      ...
      mockScore: '',
      mockResult: seeded?.[0] === 'Mock Exam' ? 'Pending' : 'N/A',
      notes: seeded?.[2] || 'Planned study day',
      morningChecklist: defaultMorningChecklist(),
      afternoonChecklist: defaultAfternoonChecklist(),
    };
  });
}
...
function cardClass(extra = '') {
  return `rounded-3xl border border-slate-200 bg-white shadow-sm ${extra}`.trim();
}
...
function TabButton({ active, onClick, children }) {
  return (
    <button
      onClick={onClick}
      className={`rounded-2xl px-3 py-2 text-xs sm:text-sm font-medium transition ${
...
function InputBase(props) {
  return (
    <input
      {...props}
      className={`w-full rounded-2xl border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 outline-none transition focus:border-slate-500 focus:ring-2 focus:ring-slate-200 ${props.className || ''}`}
    />
  );
}
...
function ButtonBase({ children, variant = 'primary', className = '', ...props }) {
  const styles =
    variant === 'outline'
...
function mockBadge(result) {
  if (result === 'Pass') return <SimpleBadge success>Pass</SimpleBadge>;
  if (result === 'Fail') return <SimpleBadge danger>Fail</SimpleBadge>;
  if (result === 'Pending') return <SimpleBadge warning>Pending</SimpleBadge>;
  return <SimpleBadge outline>N/A</SimpleBadge>;
}
...
export default function CFAStudyTrackerApp() {
  ...
  const [weeklyGoals, setWeeklyGoals] = useState(() => {
    if (typeof window !== 'undefined') {
      const saved = window.localStorage.getItem('cfa-study-tracker-weekly-goals');
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      window.localStorage.setItem('cfa-study-tracker-weekly-goals', JSON.stringify(weeklyGoals));
    }
  }, [weeklyGoals]);
  ...
  const metrics = useMemo(() => {
    ...
    const avgAccuracy = completed.length
      ...
      : 0;
    const mockDone = completed.filter((l) => l.mock === 'Yes');
    const mockCount = mockDone.length;
    const passedMocks = mockDone.filter((l) => l.mockResult === 'Pass').length;
    const failedMocks = mockDone.filter((l) => l.mockResult === 'Fail').length;
  ...
  const updateChecklist = (section, key, value) => {
    setForm((prev) => ({
      ...prev,
      [section]: {
        ...(prev[section] || {}),
  ...
  const handleAddLog = () => {
    ...
    setLogs((prev) => {
      const normalized = {
        ...form,
        mockResult: form.mock === 'Yes' ? form.mockResult || 'Pending' : 'N/A',
      };
      const exists = prev.find((l) => l.date === form.date);
      if (exists) {
        return prev.map((l) => (l.date === form.date ? normalized : l)).sort((a, b) => new Date(a.date) - new Date(b.date));
      }
      return [...prev, normalized].sort((a, b) => new Date(a.date) - new Date(b.date));
    });
  };
  ...
  const loadDay = (log) => {
    setForm({
      ...log,
      morningChecklist: log.morningChecklist || defaultMorningChecklist(),
      afternoonChecklist: log.afternoonChecklist || defaultAfternoonChecklist(),
    });
    setActiveTab('log');
  };
          ...
          <div className="flex flex-col gap-4 sm:gap-6 lg:flex-row lg:items-center lg:justify-between">
            <div>
              <div className="flex items-center gap-2 text-slate-600">
                ...
                <span className="text-xs sm:text-sm">Mobile-first study tracker</span>
              </div>
              <h1 className="mt-2 text-2xl sm:text-3xl md:text-4xl font-bold text-slate-900">CFA Level I Study Tracker</h1>
              <p className="mt-2 text-sm sm:text-base text-slate-600">Built around your study windows: 4:00 to 6:30 AM and 4:00 to 6:00 PM</p>
              <div className="mt-4 flex flex-wrap gap-2">
                <SimpleBadge dark>Exam Target: Aug 24, 2026</SimpleBadge>
                <SimpleBadge outline>Daily checklists</SimpleBadge>
                <SimpleBadge outline>Backup + import</SimpleBadge>
              </div>
            </div>
            <div className="w-full rounded-2xl bg-slate-100 p-4 sm:p-5 lg:w-80">
              <div className="mb-3 flex items-center gap-3">
                <CalendarDays className="h-5 w-5 text-slate-700" />
                <p className="font-semibold text-slate-800">Exam Countdown</p>
              </div>
              <p className="text-3xl sm:text-4xl font-bold text-slate-900">{getDaysToExam()}</p>
              ...
              <div className="mt-4 space-y-2">
                ...
                <div className="h-3 w-full overflow-hidden rounded-full bg-slate-200">
                  <div className="h-full rounded-full bg-slate-900" style={{ width: `${Math.min(100, (metrics.totalHours / totalTargetHours) * 100)}%` }} />
                </div>
              </div>
            </div>
          </div>
